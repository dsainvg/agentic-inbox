...
function MailboxSettings() {
	...
	const sections = SETTINGS_SECTIONS.filter((item) => !isWorkspace || ["groups", "memory", "automations", "audit", "team", "reliability", "backup"].includes(item.id));
		...
		<div className="w-full h-full overflow-y-auto bg-[#0f0f0f] grayscale">
			<div className="max-w-4xl mx-auto px-6 py-8 md:px-10 md:py-10">
				...
				<div className="space-y-8">
					...
					{(activeSection === "audit" || activeSection === "team" || activeSection === "reliability" || activeSection === "backup") && <OperationsSettings key={`${mailboxId}:${activeSection}`} section={activeSection} mailboxId={specificMailboxId} />}
