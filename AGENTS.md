...
- There is no configured `lint`, formatter, or `npm test` script. Run standalone tests with `node --test`:
  ...
  - `node --test tests/attachment-storage.test.mjs`
...
- `workers/index.ts` owns the main HTTP API. `workers/inbound/receive-email.ts` owns the inbound email pipeline; `workers/agent/index.ts` is the `EmailAgent` Durable Object; `workers/mcp/index.ts` is the stateless MCP surface. Feature routers live in `workers/routes/` (`audit`, `users`, `hierarchy`, `reply-forward`, `backup`, `attachments`). The React UI is in `app/`; `shared/` contains code imported by both browser and Worker code.
- Keep large frontend areas split by domain: settings UI helpers live in `app/components/settings/`, hierarchy primitives in `app/components/hierarchy/`, and the hierarchy HTTP client in `app/queries/hierarchy.ts`.
...
- Current outbound API/tool paths use optional `SMTP_USER`/`SMTP_PASS` through `workers/lib/smtp.ts`; do not assume a Cloudflare `send_email` binding is used.
- Attachment storage is optional: `ATTACHMENTS` R2 is preferred; when absent, `APPWRITE_ENDPOINT`/`APPWRITE_PROJECT_ID`/`APPWRITE_API_KEY`/`APPWRITE_BUCKET_ID` provide the fallback. If neither is configured, mail persists normally without attachment bytes. MIME/size policy is enforced in `workers/lib/attachments.ts`.
